...
namespace myteam_admin.Fenetres
{
    public partial class menuUtilisateur : UserControl
    {
        ...
        private void supprimerUtilisateur(object sender, EventArgs e)
        {
            ...
            if (utilisateur.getPoste().getGrade() >= accueil.currentUser.getPoste().getGrade())
            {
                dialogAlert alert = new dialogAlert("Vous ne pouvez pas supprimer " + utilisateur.getNom() + " " + utilisateur.getPrenom() + "\r\n car il/elle à une autorisation supérieure ou égale à la votre.");
                alert.ShowDialog();
            }
            else
            {
