...
namespace myteam_admin.Fenetres
{
    public partial class modifierUtilisateur : Form
    {
        ...
        private void button_ban_Click(object sender, EventArgs e)
        {
            ...
            if (utilisateur.getPoste().getGrade() >= menu.accueil.currentUser.getPoste().getGrade())
            {
                dialogAlert alert = new dialogAlert("Vous ne pouvez pas bannir " + utilisateur.getNom() + " " + utilisateur.getPrenom() + "\r\n car il/elle à une autorisation supérieure ou égale à la votre.");
                alert.ShowDialog();
            }
            else
            {
