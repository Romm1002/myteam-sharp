...
namespace myteam_admin.Fenetres
{
    public partial class menuPostes : UserControl
    {
        ...
        private void buttonAjoutPoste_Click(object sender, EventArgs e)
        {
            ...
            accueil.labelHeaderTitle.Text = "AJOUTER UN POSTE";
        }
        ...
        private void supprimerPoste(object sender, EventArgs e)
        {
            ...
            if(poste.getId() == 3)
            {
                dialogAlert alert = new dialogAlert("Visiteur étant le poste par défaut vous ne pouvez pas le supprimer.");
                alert.ShowDialog();
            }
            else
            {
        ...
        private void datagridviewPostes_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            Postes poste = new Postes((int)row.Cells[0].Value, row.Cells[1].Value.ToString(), int.Parse(row.Cells[2].Value.ToString()));
            poste.update();
        }
    }
}
